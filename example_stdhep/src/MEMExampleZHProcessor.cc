...
//                        ----Junping
// *****************************************************
#include "MEMExampleZHProcessor.h"
#include <iostream>
#include <sstream>
#include <iomanip>
...
MEMExampleZHProcessor aMEMExampleZHProcessor ;


MEMExampleZHProcessor::MEMExampleZHProcessor() : Processor("MEMExampleZHProcessor") {
  
  // modify processor description
  _description = "MEMExampleZHProcessor calculates the differential cross section ..." ;
...
void MEMExampleZHProcessor::init() { 
  ...
  // initialize LCMEZH with Higgs mass of 125 GeV and beam polarisations P(e-,e+) = (0.,0.)
  _zh = new LCMEZH("LCMEZH","ZH",125.,0.,0.);
  _zh->SetZDecayMode(5);
  
}

void MEMExampleZHProcessor::processRunHeader( LCRunHeader* run) { 

  _nRun++ ;
} 

void MEMExampleZHProcessor::processEvent( LCEvent * evt ) { 
  ...
  if (!hGen) {
    ...
    tupstr_gen << "sigma:sigmall:sigmarl:sigmalr:sigmarr" << ":"
	       << "mz:mzh:costheta:phi:costhetaf:phif"
	       << ends;
  ...
  _zh->SetMomentumFinal(vLortzMC);
  // matrix element can be given for each combination of initial and final helicities
  Int_t vHelLL[2] = {-1,-1};
  Int_t vHelLR[2] = {-1,1};
  Int_t vHelRL[2] = {1,-1};
  Int_t vHelRR[2] = {1,1};
  Double_t dSigmaLL = _zh->GetMatrixElement2(vHelLL);
  Double_t dSigmaLR = _zh->GetMatrixElement2(vHelLR);
  Double_t dSigmaRL = _zh->GetMatrixElement2(vHelRL);
  Double_t dSigmaRR = _zh->GetMatrixElement2(vHelRR);
  ...
  Double_t dSigma   = _zh->GetMatrixElement2();
  ...
  data_gen[3] = dSigmaLR;
  data_gen[4] = dSigmaRR;
  data_gen[5] = TMath::Sqrt(_zh->GetQ2Z());
  data_gen[6] = TMath::Sqrt(_zh->GetQ2ZH());
  data_gen[7] = _zh->GetCosTheta();
  data_gen[8] = _zh->GetPhi();
  data_gen[9] = _zh->GetCosThetaF();
  data_gen[10]= _zh->GetPhiF();
  hGen->Fill(data_gen);
...
void MEMExampleZHProcessor::check( LCEvent * evt ) { 
  // nothing to check here - could be used to fill checkplots in reconstruction processor
}


void MEMExampleZHProcessor::end(){ 

  cerr << "MEMExampleZHProcessor::end()  " << name() 
